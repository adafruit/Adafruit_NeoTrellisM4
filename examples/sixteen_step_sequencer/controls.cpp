...
void Controls::tempoOverlay(bool inc) {
  if (!_tempoHoldCount) {
    ...
    for (int i = 0; i < COLS; i++) {
      if (i == base)
        _overlayTempo[i] = remainder << 8;
      else if (i < base)
        _overlayTempo[i] = 0x00FF00;
    }
  } else
    _tempoHoldCount--;
...
void Controls::stateNormal() {
  ...
  while (trellis.available()) {
    ...
    if (e.bit.EVENT == KEY_JUST_PRESSED) {
      ...
      if (!trellis.isPressed(KEY_JUMP)) {
        ...
        } else if (e.bit.KEY <= SEQ_NUM_STEPS && sequencer.isWriting() &&
                   !trellis.isPressed(KEY_SOUND)) {
      ...
      case (KEY_LPF):
        if (filterActive != FILTER_LPF) {
          ...
          myFilterL.begin(fir_list[FILTER_LPF].coeffs,
                          fir_list[FILTER_LPF].num_coeffs);
          myFilterR.begin(fir_list[FILTER_LPF].coeffs,
                          fir_list[FILTER_LPF].num_coeffs);
      ...
      case (KEY_HPF):
        if (filterActive != FILTER_HPF) {
          ...
          _overlay[keyindex] = OVERLAY_FX_ON;
          myFilterL.begin(fir_list[FILTER_HPF].coeffs,
                          fir_list[FILTER_HPF].num_coeffs);
          myFilterR.begin(fir_list[FILTER_HPF].coeffs,
                          fir_list[FILTER_HPF].num_coeffs);
  ...
  // do our overlay
  if (trellis.isPressed(KEY_SOUND))
    _overlay[KEY_SOUND] = 0x0000FF;
  else
    _overlay[KEY_SOUND] = OVERLAY_VOICE_OFF;

  if (sequencer.isWriting())
    _overlay[KEY_WRITE] = 0x0000FF;
  else
    _overlay[KEY_WRITE] = OVERLAY_VOICE_OFF;
