...
uint32_t peak = trellis.Color(255, 0, 0);
  ...
  if (myFFT.available()) {
    ...
    for (i=0; i<8; i++) { // Column
      ...
      for(int j=0; j<=3; j++){ // Row
        ...
        if (j == 0 && n > cell_end) {
          trellis.setPixelColor(pixnum, peak);
          ...
          if (use_color == 0) {
            use_color = trellis.Color(colors[j][0], colors[j][1], colors[j][2]);
          }
        }
          ...
          trellis.setPixelColor(pixnum, use_color);
        }
        else {
